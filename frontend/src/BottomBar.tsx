    ...
    <View style={styles.wrapper} pointerEvents="box-none">
      ...
      {fabExpanded ? (
        ...
        <View style={styles.inner}>
          ...
          {/* FAB */}
          <TouchableOpacity onPress={() => setFabExpanded(true)} activeOpacity={0.85} style={styles.fab} accessibilityRole="button" accessibilityLabel="Expand chat input">
...
function NavTab({ tab, active, onPress }: { tab: typeof TABS[number]; active: boolean; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.7} style={styles.navTab} accessibilityRole="button" accessibilityLabel={`Go to ${tab.label}`}>
...
function ChatInput({ newChatText, setNewChatText, onSubmit, onClose, showClose }: {
  ...
  return (
    <View style={inputStyles.row}>
      {showClose && (
        <TouchableOpacity onPress={onClose} activeOpacity={0.8} style={inputStyles.sideBtn} accessibilityRole="button" accessibilityLabel="Close chat input">
      ...
      <View style={inputStyles.box}>
        ...
        <TouchableOpacity onPress={onSubmit} activeOpacity={0.8}
          style={[inputStyles.sendBtn, { backgroundColor: newChatText.trim() ? T.amber : T.card }]} accessibilityRole="button" accessibilityLabel={newChatText.trim() ? "Send message" : "Voice input"}>
