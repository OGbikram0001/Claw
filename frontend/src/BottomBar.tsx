    ...
    <View style={styles.wrapper} pointerEvents="box-none">
      ...
      {fabExpanded ? (
        ...
        <View style={styles.inner}>
          ...
          {/* FAB */}
          <TouchableOpacity accessibilityRole="button" accessibilityLabel="New Action" onPress={() => setFabExpanded(true)} activeOpacity={0.85} style={styles.fab}>
...
function NavTab({ tab, active, onPress }: { tab: typeof TABS[number]; active: boolean; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Tab ${tab.label}`} accessibilityState={{ selected: active }} onPress={onPress} activeOpacity={0.7} style={styles.navTab}>
...
function ChatInput({ newChatText, setNewChatText, onSubmit, onClose, showClose }: {
  ...
  return (
    <View style={inputStyles.row}>
      {showClose && (
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Close" onPress={onClose} activeOpacity={0.8} style={inputStyles.sideBtn}>
          <Ic name="close" size={20} color={T.textSec} />
        </TouchableOpacity>
      )}
      <View style={inputStyles.box}>
        <TouchableOpacity accessibilityRole="button" accessibilityLabel="Attach file" activeOpacity={0.7} style={inputStyles.attach}>
        ...
        />
        <TouchableOpacity accessibilityRole="button" accessibilityLabel={newChatText.trim() ? "Send message" : "Voice message"} onPress={onSubmit} activeOpacity={0.8}
