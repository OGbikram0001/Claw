...
export default function ChatScreen() {
  ...
  // PERFORMANCE: Use useCallback to preserve reference stability across re-renders
  // preventing unnecessary re-renders in the React.memo wrapped AgentBubble list items.
  const toggleTool = useCallback((id: number) => {
    setExpandedTools(p => ({ ...p, [id]: !p[id] }));
  }, []);

  const onApprove = useCallback((msg: any) => {
    setMessages(p => p.map(x =>
      x.id === msg.id ? { ...x, role: "agent" as const, content: "✓ Approved. Deploying…" } : x
    ));
  }, []);

  const onReject = useCallback((msg: any) => {
    setMessages(p => p.map(x =>
