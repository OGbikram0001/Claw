...
export default function ChatScreen() {
  ...
  const handleApprove = useCallback((msg: any) => {
    setMessages(p => p.map(x => x.id === msg.id ? { ...x, role: "agent" as const, content: "✓ Approved. Deploying…" } : x));
  }, []);

  const handleReject = useCallback((msg: any) => {
    setMessages(p => p.map(x => x.id === msg.id ? { ...x, role: "agent" as const, content: "Rejected. No changes made." } : x));
  }, []);
